...
export const CategoryParentIdSearchBar = () => {
  const router = useSafeRouter(useRouter);

  const onSubmit = async (input: string) => {
    if (input === "") {
      router.safePush("/categories");
      return;
    }
    try {
      const validInput = formSchema.parse({
        parentId: input.replaceAll("\t", ""),
      });
      const newSearchParams = new URLSearchParams();
      newSearchParams.set("parentId", validInput.parentId);
      router.safePush(`/categories?${newSearchParams.toString()}`);
    } catch (error: any) {
      console.log(error);
